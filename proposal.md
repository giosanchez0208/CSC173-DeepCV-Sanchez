...
- Build and train a custom semantic segmentation CNN (implemented from scratch, without using prebuilt object detection frameworks) to detect and classify individual alphanumeric characters on license plates under varied imaging conditions. [5][10]
...
- **Acquisition / Generation Strategy:**
  - Collect a diverse pool of background images to model realistic, cluttered environments (day/night, urban/rural, motion).
...
- **Custom Semantic Segmentation CNN and Training:**
  - **Architecture Design:**
    - Implement an encoder–decoder (or fully convolutional) network from scratch that takes a license plate crop as input and outputs a segmentation map where each spatial location is assigned a character class (or background). [5][10]
    - The encoder progressively downsamples features through stacked convolutional and pooling layers; the decoder upsamples and refines predictions, optionally using skip connections to preserve fine spatial detail.
  ...
  - **Loss Function:**
    ...
    - Optionally apply class weighting to handle imbalanced character frequencies across the dataset.
  - **Implementation:**
    - Implement the network from scratch in a deep learning framework (e.g., PyTorch), explicitly coding layers, loss functions, and training loops without relying on pretrained weights or object detection frameworks. [5][8]
    ...
    - Train on synthetic data and validate on held-out synthetic and a small set of real Philippine plate images. [4][7]
